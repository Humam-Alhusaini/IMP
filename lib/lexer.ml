...
let start_pos = { line_num = 1; bol_off = 0; offset = 0 }
...
let reset_bol_off pos =
  { line_num = pos.line_num; offset = pos.offset; bol_off = 0 }

let new_line pos =
  { line_num = pos.line_num + 1; offset = pos.offset + 1; bol_off = 1 }
...
let parse_char char pos : parseable_token =
  let t =
    ...
    | '.' -> PERIOD
    | errt ->
        Lexing_error (sprintf "%c does not match any known char" errt, pos)
        |> raise
  in
  (t, pos)
...
let rec charify_word lx pos =
  if at_eof lx pos |> not then begin
    ...
    match char with
    | 'a' .. 'z' | 'A' .. 'Z' | '0' .. '9' | '_' ->
        char :: (shiftr pos |> charify_word lx)
    | _ -> []
  end
  else []
...
let rec charify_num lx pos : char list =
  if at_eof lx pos |> not then begin
    ...
    | 'a' .. 'z' | 'A' .. 'Z' ->
        Lexing_error
          ("Can't end number with letter, add a space or something", pos)
        |> raise
...
let rec tokenize_symbol lx pos =
  if at_eof lx pos |> not then begin
    ...
    | _ -> Lexing_error ("< should either end with = or >", pos) |> raise
  end
  else
    Lexing_error ("< should either end with = or >, but it ended with EOF", pos)
    |> raise
