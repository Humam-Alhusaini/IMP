...
let check_and_skip ps endtok =
  ...
  | (ftok, p) :: ls ->
      if endtok = ftok then ls
      else
        Parsing_error
          ( format_tok endtok |> sprintf "Expected %s to end statement",
            (ftok, p) )
        |> raise
...
let parse_aexp ps =
  ...
  in
  match ps with
  | (LPAREN, _) :: (LIT lit, _) :: ls ->
      let toks, p = parse_binop ls (ALit lit) in
      (check_and_skip toks RPAREN, p)
  | (LIT lit, _) :: ls -> ALit lit |> parse_binop ls
  | [] -> Fatal "No tokens to parse" |> raise
  | hd :: _ ->
      Parsing_error ("aexpession did not start with LPAREN or lit", hd) |> raise

let rec parse_bexp ps =
  let rec parse_binop (ps, curr) =
    match ps with
    | (AND, p) :: ls ->
        let toks, bexp = parse_bexp ls in
        (toks, Or (curr, bexp))
    | (OR, p) :: ls ->
        let toks, bexp = parse_bexp ls in
        (toks, And (curr, bexp))
  ...
  | (NOT, _) :: ls ->
      let toks, b = parse_bexp ls in
      (toks, Not b)
  | [] -> Fatal "No tokens to parse" |> raise
  | hd :: _ ->
      Parsing_error ("bexpession did not start with LPAREN", hd) |> raise
...
and parse_def ps =
  match ps with
  | (LIT (`VAR str), _) :: (EQ, _) :: ls ->
      let ps, aexp = parse_aexp ls in
      let term = Def (str, aexp) in
      (ps, term)
  | (LIT (`VAR str), _) :: tok :: _ ->
      Parsing_error ("Missing Equal Sign in Definition", tok) |> raise
...
and parse_if ps =
  ...
  let term = If (cond, nested_term) in
  (ps, term)
...
and parse_elif ps =
  ...
  let term = Elif (cond, term1, term2) in
  (ps, term)
...
and parse_while ps =
  ...
  let term = While (cond, term) in
  (ps, term)

and parse_print ps =
  let ps, aexp = parse_aexp ps in
  let term = Print aexp in
  (ps, term)
